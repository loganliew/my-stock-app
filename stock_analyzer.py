...
sub_chart_1 = st.sidebar.selectbox("📊 副圖軌道 1：", slot_options, index=0) # 預設成交量
sub_chart_2 = st.sidebar.selectbox("📊 副圖軌道 2：", slot_options, index=1) # 預設 MACD
sub_chart_3 = st.sidebar.selectbox("📊 副圖軌道 3：", slot_options, index=2) # 預設 KD
...
def fetch_yahoo_eps(sid):
    ...
    if (
        ...
        return pd.DataFrame(eps_q.values, columns=formatted_cols, index=["季度 EPS"]).fillna("--")
    return pd.DataFrame()
...
try:
    ...
    if df_all.empty:
        ...
        # =================================================================
        st.markdown("---")
        st.subheader(f"📈 互動看板 (主圖內建深色均線，可自由疊加布林通道)")
        ...
        if show_bb:
            ...
            fig.add_trace(go.Scatter(x=df.index, y=df["BB_Mid"], line=dict(color="#FFC0CB", width=1.5, dash="dash"), name="布林中軌(20MA)"), row=1, col=1)
        ...
        for slot_choice in active_slots:
            if slot_choice == "成交量 (VOL)":
                ...
                fig.update_yaxes(title_text="成交量", row=current_track_row, col=1)
                ...
                fig.update_yaxes(title_text="MACD", row=current_track_row, col=1)
                
            elif slot_choice == "KD 隨機指標":
                fig.add_trace(go.Scatter(x=df.index, y=df["K"], line=dict(color="#FF9900", width=1.8), name="K值(快線)"), row=current_track_row, col=1)
                fig.add_trace(go.Scatter(x=df.index, y=df["D"], line=dict(color="#3399FF", width=1.8), name="D值(慢線)"), row=current_track_row, col=1)
                fig.add_hline(y=80, line_width=1, line_dash="dash", line_color="#FF4444", row=current_track_row, col=1)
                fig.add_hline(y=20, line_width=1, line_dash="dash", line_color="#00AA00", row=current_track_row, col=1)
                fig.update_yaxes(title_text="KD(9,3,3)", range=[0, 100], row=current_track_row, col=1)
                
            elif slot_choice == "RSI 相對強弱指標":
                fig.add_trace(go.Scatter(x=df.index, y=df["RSI"], line=dict(color="#FF00FF", width=1.8), name="RSI(14日)"), row=current_track_row, col=1)
                fig.add_hline(y=70, line_width=1, line_dash="dash", line_color="#FF4444", row=current_track_row, col=1)
                fig.add_hline(y=30, line_width=1, line_dash="dash", line_color="#00AA00", row=current_track_row, col=1)
                fig.update_yaxes(title_text="RSI(14)", range=[0, 100], row=current_track_row, col=1)
                ...
                if df_chip_timeline.empty:
                    ...
                    mapping_key = {
                        "外資買賣超 (張)": ("Foreign", "外資買賣超"),
                        "投信買賣超 (張)": ("Trust", "投信買賣超"),
                        "自營商買賣超 (張)": ("Dealer", "自營商買賣超"),
                        "三大法人合計 (張)": ("Total", "法人合計")
                    }
                    target_col, label_text = mapping_key.get(slot_choice, ("Foreign", "外資買賣超"))
        ...
        fig.update_layout(
            ...
            title=dict(text=f"📈 {stock_name} ({stock_id}) - 全方位指標分析", font=dict(size=22, family="Microsoft JhengHei")),
            xaxis_rangeslider_visible=False, height=900, margin=dict(l=50, r=50, t=50, b=50), showlegend=True,
        ...
        if is_tw_stock:
            ...
            try:
                ...
                chip_df = fm_loader.taiwan_stock_institutional_investors(stock_id=stock_input, start_date=chip_start_date, end_date=chip_end_date)
                ...
                if not chip_df.empty:
                    ...
                    def color_buy_sell(val):
                        if isinstance(val, (int, float)):
                            return f"color: {'#FF3333' if val >= 0 else '#00AA00'}; font-weight: bold;"
                        return ""
